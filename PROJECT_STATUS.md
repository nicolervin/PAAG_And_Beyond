...
- **Assembly grid** — Provides a project-wide, Fishbone-section grid of real EBOM categories by active official model. Each model cell accepts one complete assembly number through its **Part number** field, alongside direct model mappings, shared continuously synchronized Installed sections, active feature headers with saved visibility, nested mini-BOM quantity editing, confirmed custom-grid deletion entry points, and Details access to images, the mini-BOM, retained Task 04 catalog metadata and nesting, and the full assembly deletion workflow. Images are the first/default Details tab; Mini-BOM is second; the former Feature rules tab is hidden.
...
- The committed `unittest` suite currently passes 71 tests covering planning-scenario behavior, scope selectors, shared direct-entry sorting and deletion-target interpretation, decimal Fishbone quantity migration and export, assembly catalog and grid isolation/mapping/synchronization/rollback/reset behavior, combined six-reference Fishbone section continuity re-pointing, saved-state Undo, cross-scenario Yamazumi and category-collision prevention, dual Built/Installed Fishbone listing, duplicate-feature-rule rejection, strict custom-component JSON payloads, deleted blank grid drafts, consecutive draft mini-BOM additions, and Pin Map data derivation. AppTest smoke coverage includes Parts to fishbone, Yamazumi, Process at a Glance, and Assembly grid.
