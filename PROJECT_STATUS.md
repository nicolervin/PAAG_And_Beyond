...
- **Parts to fishbone** — Builds the product's assembly framework, including main sections and nested subassemblies. Collaborators can place approved catalog parts into that framework, record quantities and uses, order them, and view the result as an interactive fishbone with images and feature filtering. Exact sections can also carry project-wide feature/choice qualifying conditions that combine with OR, remain visibly flagged and fail open when stale, and combine with existing part-rule membership for the visual's feature filter only.
...
- The committed `unittest` suite currently passes 22 tests covering planning-scenario behavior, scope selectors, shared direct-entry sorting and deletion-target interpretation, Pin Map data derivation, and Fishbone section qualifying-condition validation, filtering, staleness, deletion, and foreign-key behavior.
