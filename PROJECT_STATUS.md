...
- **Quality** — Uses a Scenario-aware page with **Requirements repository**, **PFMEA**, and placeholder **Control Plan** tabs. The repository remains project-wide with direct entry, relationship-safe deletion, explicit publication, assignment management, and a cross-scenario linked-requirements view. PFMEA is scenario-specific and presents the approved 19-column workbook body as a flat editor over normalized failure-mode, Effect, Cause, structured Control, risk-row, and Action records. Item # shows Process Pitch; Process Function shows the Process at a Glance Work Element; stable IDs remain hidden. Cause-level Prevention and Detection fields display friendly structured tags from applicable published Quality assignments or separate project-wide manual catalogs. They support native same-column Ctrl+C/Ctrl+V replacement, Cause-level propagation, compatibility confirmation for step-specific Quality sources, inline omission warnings for inactive manual sources, Undo, and atomic Save & Refresh persistence. The row-specific panel remains available for deliberate selection and previewed Cause-to-Cause copying. Ordinary cells retain spreadsheet copy/paste, while the governed duplicate-line workflow creates independent PFMEA drafts with fresh saved identities, clears completion evidence, and carries only valid same-step Quality references and active manual controls. Draft Process Function changes confirm removal of incompatible Quality controls. Source updates/removals are review-flagged without changing Detection ratings; Quality unlink and catalog deletion disclose and audit their cascades. Scenario cloning remaps Quality sources and reuses manual options. Ratings, RPN recalculation, high-risk view, Process-source review, confirmed deletion, audit logging, and the single grouped History expander remain supported. Legacy free-text controls are discarded once with editor-attributed count-only evidence. Control Plan generation is not implemented.
...
- The navigation contains 14 screens: Overview, Questions and concerns, Import PITS and export, Model definitions, Parts Catalog, Parts to fishbone, Yamazumi, Process at a Glance, Pin Map, the working Quality repository, and four non-persistent Functional Reviews shells. The established screens and Process at a Glance detail dialog have passed prior smoke checks; Pin Map and Quality have focused page smoke coverage.
- All Python files pass a syntax/import compilation check with the installed packages.
- The committed `unittest` suite currently passes 94 tests covering planning scenarios, scope selectors, shared table behavior, Pin Map derivation, Quality/PFMEA page smoke behavior, repository persistence, structured PFMEA controls and catalog constraints, governed PFMEA copying, count-only legacy migration, source-review/cascade behavior, scenario cloning, Item #/Process Function mapping, RPN behavior, atomic saves, duplicate prevention, and Process-step rebalance durability.
