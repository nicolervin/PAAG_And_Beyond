...
- **Source Code:** `source_code` is an editable project-wide text value containing the numeric PITS source code from tracker column T. It is populated for PITS-imported parts, backfilled from stored PITS source payloads when available, and remains blank for manually created parts until entered. The Parts Catalog displays the approved code legend as hover help: `1 - Purchased Part`, `+1 - Purchased Asm`, `2.4 - From AP3`, `3 - Mfg Part`, `5 - Asm (Disassembly no)`, `6 - Asm (Disassembly yes)`, and `8 - Component of Purchased Asm`.
