...
- **Purpose:** Compute a human-readable, traceable **Op ID** for each Process at a Glance Work Element wherever people need to reference that work, initially in the main Process at a Glance table and the separately planned Phase 3 inline pitch-level visual summary. The Op ID is derived at display time and is never persisted, following the existing derived-view approach used by Pin Map and the Process Ergonomics risk tag.
...
- **Storage:** No new table, field, cached value, or backfill is introduced. Op IDs are computed fresh whenever displayed. Add one shared store-layer batch function, named consistently with existing conventions, that efficiently computes Op ID display values for multiple `work_elements` rows. A single-record helper may delegate to that batch function. The shared implementation must support the Process at a Glance table, the future Phase 3 inline visual summary, and any separately approved future consumer without duplicating traversal logic in page code.
