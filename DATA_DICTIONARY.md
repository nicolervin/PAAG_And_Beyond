...
- **Review status:** **Schema and interaction design confirmed by the project owner on September 2, 2026. Implementation has not started and requires a separate explicit go-ahead.**
...
- **Connections:** Categories connect to `assembly_sections`. Direct mappings connect `assembly_grid_categories` to active `project_models` and `manufacturing_assemblies`. Header rows derive from active `complexity_features` and `model_feature_values`; visibility preferences affect display only. Component rows continue using `manufacturing_assembly_components.fishbone_assignment_id` -> `fishbone_part_assignments.part_id` -> `parts`. `manufacturing_assembly_feature_rules` remains intact, independent, and neither generated nor modified by direct grid mappings.
...
| `root_number` | `TEXT NOT NULL` | Stores the category root, such as `290D5251`, used only to accelerate assembly-number entry. |
...
Enforce `UNIQUE(project_id, section_id, ebom_name)` and `UNIQUE(project_id, section_id, display_name)`. Store validation trims names and root numbers, requires all three values, compares names case-insensitively for duplicates, and confirms both section references belong to the project. `root_number` is not required to be unique because separate EBOM categories may legitimately share a numbering family.
...
Only `assembly_id` is persisted. The grid defaults a cell to suffix entry against the category's `root_number`, so entering `G020` against `290D5251` resolves to the final assembly number `290D5251G020`. A per-cell full-number override accepts an unrelated final number when the EBOM variant does not follow the root pattern. Entry mode, typed suffix, and split root/suffix values remain in the unsaved component draft and are never stored in this table or on `manufacturing_assemblies`.

If a final assembly number already exists in the project, the save reuses it only when its Built section equals the category section and its Installed section is compatible with the category's shared value. An assembly built in a different section or carrying a conflicting Installed section is blocked with a plain-language message. A new final number creates a minimal active `manufacturing_assemblies` row in the same transaction: `assembly_number` is the resolved final number, `name` initially copies the category display name, `make_buy` remains the compatibility blank until deliberately classified, `built_section_id` comes from the category, and `installed_section_id` comes from the category or remains null. System UUIDs and UTC timestamps are supplied automatically. The richer detail workflow may later edit the friendly name, Make / buy, nesting, images, and other existing assembly fields.
...
- A Components v2 grid renders sticky category fields, model columns, and indented component rows. It stores no business data directly and returns a JSON-serializable draft to Python. Python performs all project, section, uniqueness, quantity, and foreign-key validation before any write.
...
- Each mapped cell expands only that assembly's existing `manufacturing_assembly_components` rows. There is no category-level component list or named component slot. Part number and Quantity remain separate columns.
...
After the separate implementation go-ahead, first back up `data/paag.db`, then clear all current `manufacturing_assemblies` rows and their dependent components, feature rules, images, and owned assembly-image files in one validated transaction. Existing foreign-key effects on dormant policies, material options, and target-assembly links must be included even when their current counts are zero. Preserve Parts Catalog records, Fishbone uses and sections, models, features, and all non-assembly uploads. Record one `Assemblies catalog` / `Prototype data reset` audit event per affected project with deleted counts and the timestamp. Do not drop or rebuild the Task 04 tables. New Task 09 tables begin empty, so every section grid starts empty rather than deriving mappings or categories from pre-reset assembly data.
