...
- **Purpose:** Stores one scenario-specific Potential Failure Mode for a Process at a Glance step, its reviewed Classification (blank, Product Safety, or Critical Quality), read-only Process Function snapshots, and source fingerprints used to flag later upstream changes without replacing reviewed PFMEA evidence. Exact legacy `Safety` classifications migrate to `Product Safety` across the project in one atomic, editor-attributed operation that preserves stable IDs and related PFMEA data.
