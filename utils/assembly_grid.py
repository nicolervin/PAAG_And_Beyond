...
<div class="grid-shell">
  <div class="toolbar"><button id="add-category" type="button">＋ Add category</button></div>
...
.sticky-2 { position:sticky; left:203px; z-index:4; min-width:145px; max-width:145px; }
.sticky-3 { position:sticky; left:361px; z-index:4; min-width:180px; max-width:180px; }
thead .sticky-1, thead .sticky-2, thead .sticky-3 { z-index:8; background:var(--st-secondary-background-color); }
...
.entry-mode { display:flex; gap:8px; align-items:center; margin-bottom:4px; color:var(--st-secondary-text-color); font-size:.72rem; }
.entry-mode input { width:auto; }
...
export default function(component) {
  ...
  const valueFor = (category, modelId) => {
    category.cells ||= {}
    category.cells[modelId] ||= {mapping_id:'', assembly_id:'', assembly_number:'', mode:'suffix', components:[]}
  ...
  const render = () => {
    let html = '<thead><tr><th class="sticky-1">Category</th><th class="sticky-2">Root number</th><th class="sticky-3">Installed section</th>'
    ...
    for (const feature of features) {
      html += `<tr class="feature-row"><th class="sticky-1">${escapeHtml(feature.label)}</th><th class="sticky-2"></th><th class="sticky-3"></th>`
    ...
    draft.forEach((category, categoryIndex) => {
      ...
      html += `<td class="sticky-2"><input data-field="root_number" value="${escapeHtml(category.root_number)}" placeholder="Root"></td>`
      html += `<td class="sticky-3"><select data-field="installed_section_id">${sectionOptions(category.installed_section_id || '')}</select></td>`
      let priorNumber = ''
      models.forEach(model => {
        ...
        const displayValue = cell.mode === 'full' ? cell.assembly_number : (cell.suffix ?? (cell.assembly_number.startsWith(category.root_number || '') ? cell.assembly_number.slice((category.root_number || '').length) : cell.assembly_number))
        html += `<td class="${repeated ? 'merged' : ''}" data-model="${escapeHtml(model.id)}"><label class="entry-mode"><input type="checkbox" class="full-mode" ${cell.mode === 'full' ? 'checked' : ''}> Full override</label><input class="assembly-entry" value="${escapeHtml(displayValue)}" placeholder="${cell.mode === 'full' ? 'Full assembly number' : 'Suffix'}"><div class="cell-actions">${cell.assembly_id ? '<button class="details" type="button">Details</button><button class="danger clear-mapping" type="button">Clear</button>' : ''}</div></td>`
      })
      html += '</tr>'
      const hasComponents = models.some(model => (valueFor(category, model.id).components || []).length)
      if (hasComponents) {
        html += `<tr class="component-row" data-category="${categoryIndex}"><td class="sticky-1 component-label">↳ Mini-BOM components</td><td class="sticky-2"></td><td class="sticky-3"></td>`
        models.forEach(model => {
          const cell = valueFor(category, model.id)
          html += '<td>' + (cell.components || []).map((item, componentIndex) => `<div class="component" data-component="${componentIndex}"><span>${escapeHtml(item.part_number)}</span><input class="component-quantity" type="number" min="0.000001" step="0.01" value="${escapeHtml(item.quantity)}"><button class="danger delete-component" type="button">×</button></div>`).join('') + '</td>'
        })
        html += '</tr>'
      }
    })
    ...
    table.querySelectorAll('.category-row').forEach(row => {
      ...
      row.querySelectorAll('[data-field]').forEach(input => input.onchange = () => {
        ...
        if (input.dataset.field === 'root_number') {
          Object.values(draft[categoryIndex].cells || {}).forEach(cell => {
            if (cell.mode !== 'full') cell.assembly_number = `${input.value}${cell.suffix || ''}`
          })
        }
        emitDraft()
      })
      row.querySelector('.delete-category').onclick = () => setTriggerValue('delete_category', {category_id:draft[categoryIndex].id, category_index:categoryIndex, display_name:draft[categoryIndex].display_name})
      row.querySelectorAll('td[data-model]').forEach(cellElement => {
        ...
        const mode = cellElement.querySelector('.full-mode')
        const entry = cellElement.querySelector('.assembly-entry')
        mode.onchange = () => {
          cell.mode = mode.checked ? 'full' : 'suffix'
          if (cell.mode === 'suffix') {
            cell.suffix = cell.assembly_number.startsWith(draft[categoryIndex].root_number || '')
              ? cell.assembly_number.slice((draft[categoryIndex].root_number || '').length)
              : ''
            cell.assembly_number = `${draft[categoryIndex].root_number || ''}${cell.suffix}`
          }
          render(); emitDraft()
        }
        entry.onchange = () => {
          if (cell.mode === 'full') { cell.assembly_number = entry.value.trim(); cell.suffix = '' }
          else { cell.suffix = entry.value.trim(); cell.assembly_number = `${draft[categoryIndex].root_number || ''}${cell.suffix}` }
          emitDraft()
        }
    ...
    table.querySelectorAll('.component-row').forEach(row => {
      ...
      row.querySelectorAll('td').forEach(cellElement => {
        const modelColumn = cellElement.cellIndex - 3
        ...
        cellElement.querySelectorAll('.component').forEach(componentElement => {
          ...
          componentElement.querySelector('.component-quantity').onchange = event => { cell.components[componentIndex].quantity = event.target.value; emitDraft() }
          componentElement.querySelector('.delete-component').onclick = () => setTriggerValue('delete_component', {assembly_id:cell.assembly_id, component_id:cell.components[componentIndex].id, part_number:cell.components[componentIndex].part_number})
        })
      })
    })
  }
  addButton.onclick = () => {
    draft.push({id:'', ebom_name:'', display_name:'', root_number:'', installed_section_id:'', sequence:(draft.length + 1) * 10, cells:{}})
    emitDraft(); render()
  }
  render()
}
...
_ASSEMBLY_GRID = st.components.v2.component(
    "paag_assembly_grid_v1", html=_HTML, css=_CSS, js=_JS
)
...
def assembly_grid(
    *,
    ...
    current_draft = component_state.get("draft", draft)
