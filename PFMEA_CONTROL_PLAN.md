...
| Classification | Explicit PFMEA classification | Implemented in `pfmea_entries.class_code` with approved choices blank, Product Safety, or Critical Quality; exact legacy `Safety` values migrate project-wide with editor-attributed audit evidence | Active Phase 2 storage; no Yamazumi flag is inferred |
