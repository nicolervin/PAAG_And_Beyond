...
Selection is transient and does not write data by itself. On editable tables, the native checkboxes expose deletion intent through the editor's `deleted_rows` state; they are not a supported general-purpose bulk-selection event. On tables with an approved deletion workflow, the native Delete action converts that intent into a pending confirmed deletion. Editable tables that need an ordinary bulk action must add a compact read-only native-selection surface for that action unless Streamlit later provides supported editor selection events. Filters define which rows are currently visible and therefore which rows the upper-left checkbox selects.

Use the shared table conventions in `utils/table_ui.py`: read-only tables use `selectable_dataframe()` and editable tables use native deletion selection through `num_rows="dynamic"` or `num_rows="delete"`. Use `native_selected_rows()` only to resolve that editable-table deletion intent. Show the native Delete toolbar action only when the editor is connected to an approved confirmation workflow. Do not add a named checkbox column or a separate **Select all** control to imitate selection. Where an editable-table business workflow needs ordinary single- or multi-row selection, place a compact `selectable_dataframe()` surface near that action and keep its hidden stable identifiers separate from friendly display values.
...
- Use `selectable_dataframe()` for read-only tables and ordinary bulk-action selection. Editable-table native checkboxes expose deletion intent only; never add a named `CheckboxColumn` or separate Select all control.
...
The unit control uses an explicit **Save & Refresh** action, records old/new values in Yamazumi History, and refuses to change units while the work-element editor has unsaved changes. Labels and workbook exports name the active unit.
